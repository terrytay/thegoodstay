...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      const thisMonthBookings = bookings.filter(
        (booking) => new Date(booking.created_at) >= currentMonth
      ).length;
      ...
      const thisMonthCustomers = customers.filter(
        (customer) => new Date(customer.created_at) >= currentMonth
      ).length;
      ...
      orders.forEach((order) => {
        if (order.customer_email) {
          customerOrderCounts.set(
            ...
            (customerOrderCounts.get(order.customer_email) || 0) + 1
          );
        }
      });
      ...
      const lowStockProducts = products.filter(
        (product) => product.stock_quantity <= 5 && product.stock_quantity > 0
      ).length;
  ...
  const bookingsChange = calculatePercentageChange(
    ...
    analytics.bookings.lastMonth
  );
  const customersChange = calculatePercentageChange(
    analytics.customers.thisMonth,
    analytics.customers.total - analytics.customers.thisMonth
  );
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              >
                <TrendingUp
                  className={`h-4 w-4 ${
                    !ordersChange.isPositive && "rotate-180"
                  }`}
                />
                <span className="text-sm font-medium">
                  {ordersChange.change.toFixed(1)}%
                </span>
                <span className="text-xs text-neutral-500">vs last month</span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div
                className={`flex items-center space-x-1 mt-2 ${
                  customersChange.isPositive ? "text-green-600" : "text-red-600"
                }`}
              >
                <TrendingUp
                  className={`h-4 w-4 ${
                    !customersChange.isPositive && "rotate-180"
                  }`}
                />
                <span className="text-sm font-medium">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium text-yellow-900">
                    {analytics.products.lowStock} products low in stock
                  </p>
                  <p className="text-sm text-yellow-700">
                    Items with 5 or fewer remaining
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium text-red-900">
                    {analytics.products.outOfStock} products out of stock
                  </p>
