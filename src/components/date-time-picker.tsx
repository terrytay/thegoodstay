...
export default function DateTimePicker({
  ...
  const isDateValid = (date: Date) => {
    ...
    if (dateToCheck.getTime() === today.getTime()) {
      ...
      latestBookingToday.setHours(parseInt(bookingSettings.booking_end_time?.split(':')[0] || '17'), 
                                   parseInt(bookingSettings.booking_end_time?.split(':')[1] || '0'))
      
      const minAdvanceTime = new Date(now.getTime() + (bookingSettings.min_advance_hours * 60 * 60 * 1000))
          ...
          <div className="absolute z-50 mt-2 bg-white rounded-2xl shadow-xl border border-neutral-200 p-4 w-full min-w-[320px]">
            ...
            <div className="mt-4 p-3 bg-amber-50 rounded-lg border border-amber-200">
              <div className="flex items-center space-x-2 text-sm text-amber-700">
                ...
                <span>Bookings must be at least {bookingSettings.min_advance_hours} hours in advance</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="absolute z-50 mt-2 bg-white rounded-2xl shadow-xl border border-neutral-200 w-full max-h-64 overflow-y-auto">
            <div className="p-3 border-b border-neutral-100 bg-neutral-50">
              ...
              <p className="text-xs text-neutral-600">
                {bookingSettings.booking_start_time} - {bookingSettings.booking_end_time}
              </p>
            </div>
            ...
            <div className="p-2">
              {availableTimeSlots.map((timeSlot) => {
                ...
                return (
                  ...
                  >
                    {formatTime(timeSlot)}
                    {!available && <span className="ml-2 text-xs">(Too soon)</span>}
                  </button>
                )
              })}
            </div>
