 ...
 */
export async function getBlockedSlotsForDate(date: string): Promise<CalendarEvent[]> {
  try {
...
export async function getAvailableTimeSlotsForDate(date: string): Promise<TimeSlot[]> {
  try {
    ...
    for (let hour = 7; hour <= 22; hour++) {
      ...
      const slotEnd = `${date}T${slotEndHour.toString().padStart(2, '0')}:00:00+08:00`;
 ...
 */
export async function ensureCalendarAccess(): Promise<{ success: boolean; error?: string }> {
  try {
    const calendar = createCalendarClient();
    
    // Check if calendar is already accessible
    const currentList = await calendar.calendarList.list();
    const existingCalendar = currentList.data.items?.find(cal => 
